...
function main($input){
	...
	$lines = explode ("\n",$input); // split all lines
	$t = 0;	
	foreach ($lines as $elem){ //process each line
		echo "line: ".$elem."<br>";		
		//probably this could have been done a lot faster with regex but... noone knows regex.
		//
		//Just doing a str_replace with the spelt numbers is not gonna work because some spelt numbers share characters
		//to replace the spelt numbers we'll go through each line character by character
		//search the spelt numbers searching x characters from the position of current character, and construct a new line with 
		//the found numbers, then the process the line the same as the rest of the first step.
		//
		//please keep that elf away from the machine next time
		$l = strlen($elem);
		$newline = "";

		for ($e = 0; $e < $l; $e++){ //we'll check each character from the line
			$char = substr($elem, $e, 1); 
			if (is_numeric($char)){ // if the character is a integer number, we'll just append it straight to the new line
				$newline = $newline.$char;
			} else {
				$compare = "";
				$p = 0;
				foreach ($numbers as $number){
					//now we're gonna check if starting from the current position, there's a number from the list of spelt numbers
					//this is probably going to be slow
					$compare = substr($elem, $e, strlen($number));
					
					if ($compare == $number){
						//if we found a spelt number, we add the corresponding integer to the new line
						//and exit the comparison loop to the next character in the line
						$newline = $newline.$replace[$p];
						break;
					} 
					$p++;
				}
				//at this point we didn't found any spelt number starting from the current character 
				//we can move on to the next one
			}
		}

		//at last, we set the new constructed line and the rest of the code proceeds as the previous part
		echo "found numbers: ".$newline."<br>";	
		$elem = $newline;
		
		$line = str_split(trim($elem)); //split all characters of current line
		$n = "";
		for ($i = 0; $i < 2; $i++) { //execute this only two times.
			foreach($line as $char){
				if (is_numeric($char)){
					$n = $n.$char; // if the value is a number, append it to n
					break;
				}
			}
			$line = array_reverse($line); // reverse the order of the line 
		}	
	echo ">>: ".$n."<br>";	
	$t += intval($n); // sum t + numeric value of n
	}
...
$doc = "gtlbhbjgkrb5sixfivefivetwosix
