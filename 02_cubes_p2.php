...
// These elfs sure like some strange games! but at least this day's puzzle was easier
// lots of fun, lots of loops, lots of explosions too!

function main($input){
	$sum = 0;
	$lines = explode("\n",$input); //split the document on lines
	foreach ($lines as $line){ // we go through each line
		$max = array("red" => 0,"green"=>0,"blue"=>0); //this time we initialize each color on the minimum for each line
		list($game, $sets) = explode(": ",$line); //separate the game id from the sets
	
		$sets = explode("; ",$sets); //separate all the sets
		foreach ($sets as $set){ //we go through each set
			$elems = explode(", ",$set); // separate all the colors on the set
			foreach ($elems as $elem){ //go through each color
				list($value, $color) = explode(" ",$elem);
				if ($value > $max[$color]){ // this time we just check if the current value of the color is greater than the minimum
					$max[$color] = $value;  // and if it is we replace the actual minimum value
				}
			}
		}
		
		// we multiply the maximum of all the colors and add the value to the final sum
		$power = ($max['red'] * $max['green'] * $max['blue']); 
		echo "Game $id: power: $power <br>";
		$sum = $sum + $power;
	}
	return $sum;
}
...
$doc = "Game 1: 4 red, 1 green, 15 blue; 6 green, 2 red, 10 blue; 7 blue, 6 green, 4 red; 12 blue, 10 green, 3 red
