...
function main($input){
	$limit = array("red" => 12,"green"=>13,"blue"=>14); //set the max values of each color
	...
	foreach ($lines as $line){ // we go through each line
		...
		foreach ($sets as $set){ //we go through each set
			...
			foreach ($elems as $elem){ //go through each color
				...
				if ($value > $limit[$color]){ //if the value of the current color is above the limit
					$possible = false;        //then the game is not possible
				}
			}
		}
		
		if ($possible == true){
		echo "Possible game: $id<br>"; //if the game is possible, then we sum the current id to the final sum
			$sum = $sum + $id;
		}
		$id++;
	}
